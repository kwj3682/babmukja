...
public class MemberController {
	...
	public void passMail(Member member) throws UnsupportedEncodingException, MessagingException {
		System.out.println("비밀번호 인증메일 들어옴");
		MailHandler sendMail = new MailHandler(mailSender);
		StringBuffer sb = new StringBuffer();
		// Subject -> 제목
		// Text -> 내용
		...
		sendMail.setText(sb.append("<h2>밥먹자 이메일 인증번호 입니다!</h2>")
				.append("밥먹자 이메일 인증번호는 " + '[' + cert + ']' + " 입니다.").toString());
		sendMail.setFrom("babmukja@babmukja.com", "밥먹자");
		sendMail.setTo(member.getMemEmail());
		// 멀티 쓰레드 사용
        ExecutorService executor = Executors.newWorkStealingPool();
	...
	@ResponseBody
	public void checkNum(Member member) {
		member.setCertification(member.getCertification());
		System.out.println(member.getCertification());
		service.updateCertification(member);
	}

	// 인증번호 확인
	@RequestMapping("/checkecertification.do")
	@ResponseBody
